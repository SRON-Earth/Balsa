...
def run(run_path, train_data_filename, train_label_filename, test_data_filename, test_label_filename, *,
    ...
    balsa_path = pathlib.Path(config["path"])
    ...
    result = run_program(balsa_path / "balsa_train", *args, log=True, time_file="train.time", cwd=run_path)
    ...
    result = run_program(balsa_path / "balsa_classify", *args, "jigsaw.model", str(test_data_filename), "labels.bin", log=True, time_file="test.time", cwd=run_path)
