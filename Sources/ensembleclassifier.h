...
public:
  ...
  unsigned int classifyAndVote( FeatureIterator pointsStart, FeatureIterator pointsEnd, VoteTable &table ) const
  {
      ...
      if ( m_maxWorkerThreads > 0 ) return classifyAndVoteMultiThreaded( pointsStart, pointsEnd, table );
      else return classifyAndVoteSingleThreaded( pointsStart, pointsEnd, table );
  }
